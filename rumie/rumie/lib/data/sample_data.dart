import 'package:flutter/material.dart';

import '../models/roommate.dart';
import '../models/trait.dart';

/// Placeholder data. Replace with a real API/Firebase/etc. when you wire up a backend.
const List<Roommate> sampleRoommates = [
  Roommate(
    name: 'Maya',
    age: 24,
    emoji: '🌸',
    budget: 1200,
    location: 'Mission District',
    bio: 'Grad student, loves plants and quiet evenings. Looking for a chill space to call home.',
    traits: [
      Trait(title: 'Very tidy', color: Color(0xFF3B82F6)),
      Trait(title: 'Night owl', color: Color(0xFF6366F1)),
      Trait(title: 'Has a cat', color: Color(0xFF10B981)),
      Trait(title: 'Studious', color: Color(0xFFFB923C)),
    ],
    gradient: [Color(0xFFEC4899), Color(0xFF8B5CF6)],
  ),
  Roommate(
    name: 'Jordan',
    age: 27,
    emoji: '🎮',
    budget: 1500,
    location: 'SoMa',
    bio: 'Software engineer. Big fan of weekend coffee runs and weekday silence.',
    traits: [
      Trait(title: 'Works from home', color: Color(0xFF3B82F6)),
      Trait(title: 'Early bird', color: Color(0xFFF59E0B)),
      Trait(title: 'Non-smoker', color: Color(0xFF10B981)),
      Trait(title: 'Gamer', color: Color(0xFF8B5CF6)),
    ],
    gradient: [Color(0xFF8B5CF6), Color(0xFFEC4899)],
  ),
  Roommate(
    name: 'Aria',
    age: 22,
    emoji: '🎨',
    budget: 900,
    location: 'Oakland',
    bio: 'Art student. I bake a lot. Sometimes paint at 2am. Sorry not sorry.',
    traits: [
      Trait(title: 'Creative', color: Color(0xFFF472B6)),
      Trait(title: 'Loves cooking', color: Color(0xFFF97316)),
      Trait(title: 'Night owl', color: Color(0xFF6366F1)),
      Trait(title: 'Vegetarian', color: Color(0xFF10B981)),
    ],
    gradient: [Color(0xFFF472B6), Color(0xFFA78BFA)],
  ),
  Roommate(
    name: 'Sam',
    age: 29,
    emoji: '🧘',
    budget: 1800,
    location: 'Berkeley',
    bio: 'Yoga teacher, dog parent, plant collector. Looking for someone calm and respectful.',
    traits: [
      Trait(title: 'Zen vibes', color: Color(0xFF60A5FA)),
      Trait(title: 'Has a dog', color: Color(0xFFF59E0B)),
      Trait(title: 'Early bird', color: Color(0xFFFDE68A)),
      Trait(title: 'Very tidy', color: Color(0xFF3B82F6)),
    ],
    gradient: [Color(0xFFC084FC), Color(0xFFF472B6)],
  ),
  Roommate(
    name: 'Devon',
    age: 25,
    emoji: '🎵',
    budget: 1100,
    location: 'Outer Sunset',
    bio: 'Musician + barista. Headphones are my love language. Quiet apartment, loud songs.',
    traits: [
      Trait(title: 'Music lover', color: Color(0xFFEC4899)),
      Trait(title: 'Night owl', color: Color(0xFF6366F1)),
      Trait(title: 'Loves cooking', color: Color(0xFFF97316)),
      Trait(title: 'Non-smoker', color: Color(0xFF10B981)),
    ],
    gradient: [Color(0xFFEC4899), Color(0xFFC084FC)],
  ),
];
